...
public interface BookingRepository extends JpaRepository<Booking, Integer> {
    ...
    Boolean existsByBookerIdAndItemIdAndStatusAndEndBeforeOrderByEndDesc(Integer bookerId, Integer itemId, Status status, Instant endTo);
    ...
    Optional<BookingPeriod> findNextBookingByItemId(@Param("itemId") Integer itemId, @Param("now") Instant now);

    @Query("SELECT new ru.practicum.shareit.booking.dto.BookingPeriod(b.start, b.end) " +
            "FROM Booking b WHERE b.item.id = :itemId AND b.end < :now ORDER BY b.end DESC")
    Optional<BookingPeriod> findLastBookingByItemId(@Param("itemId") Integer itemId, @Param("now") Instant now);
