...
public class BookingServiceImpl implements BookingService {
    ...
    public BookingDto createBookingRequest(Integer bookerId, NewBookingRequest bookingRequest) {
        ...
        if (booking.getEnd().isBefore(Instant.now()) || booking.getStart().isBefore(Instant.now())) {
    ...
    private List<Booking> stateSwitch(State state1, List<Booking> bookings) {
        return switch (state1) {
            case PAST -> bookings.stream().filter(b -> b.getEnd().isBefore(Instant.now())).toList();
            case FUTURE -> bookings.stream().filter(b -> b.getStart().isAfter(Instant.now())).toList();
            case CURRENT -> bookings.stream()
                    .filter(b -> b.getEnd().isAfter(Instant.now()))
