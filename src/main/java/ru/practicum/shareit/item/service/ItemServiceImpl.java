...
public class ItemServiceImpl implements ItemService {
    ...
    public CommentDto createComment(NewCommentRequest commentDto, Integer itemId, Integer userId) {
        ...
        //todo - раскомментирую и даю пройти всем запросам на создание комментариев. с этой обработкой падают все запросы
    ...
    public ItemDtoCommentBooking get(Integer itemId, Integer userId) {
        ...
        if (userId.equals(item.getOwner().getId())) {
            next = bookingRepository.findNextBookingByItemId(itemId, Instant.now()).orElse(null);
            last = bookingRepository.findLastBookingByItemId(itemId, Instant.now()).orElse(null);
        }
    ...
    public List<ItemDto> search(String text) {
        ...
        return itemRepository.findAllByNameOrDescriptionContaining(text).stream()
    ...
    private Booking getNext(List<Booking> bookings, Integer itemId) {
        ...
        return bookings.stream()
                .filter(booking -> booking.getItem().getId().equals(itemId))
                .filter(booking -> booking.getStart().isAfter(now)) // Брони с датой начала в будущем
